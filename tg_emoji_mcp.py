...
# Version 0.4.0 - Performance & UI Overhaul
VERSION = "0.4.0"
...
async def search_and_select_emoji(
    ...
    """
    Search for custom Telegram emojis and open a browser UI for user selection. 
    IMPORTANT: Use only basic Unicode emojis as search terms (e.g. ❌, 🔴, 💎).
    """
    ...
    async with app:
        try:
            ...
            if not ids: return {"error": "No symbols found. Use ONLY Unicode icons."}
            ...
            for base, items in grouped.items():
                ...
                for i in items:
                    ...
                    items_html += f"""<div class='card' onclick='t("{i['id']}")' id='c_{i['id']}'>
                        <div class='checkbox-box'><input type='checkbox' id='i_{i['id']}' data-id='{i['id']}' data-base='{base}'></div>
                        {media}
            ...
            html = f"""<!DOCTYPE html><html><head><meta charset='utf-8'><script src='https://cdnjs.cloudflare.com/ajax/libs/bodymovin/5.12.2/lottie.min.js'></script><style>
                ...
                <div class='header'><div><h2 style='margin:0'>Emoji Selection</h2></div><button onclick='s()' id='sub' class='btn'>Confirm Selection</button></div>
                {sections_html}
                <script>
                    function t(id){{
                        const c=document.getElementById('c_'+id), i=document.getElementById('i_'+id);
                        i.checked=!i.checked; c.classList.toggle('selected',i.checked);
                    }}
                    async function s(){{
                        ...
                        if(!res.length) return alert('Please select at least one!');
            ...
            try:
                ...
                selections = raw_res.get("selections", [])
                # Map results for LLM
                mapping = {}
...
async def search_emoji_auto(emoticons: list[str], limit: int = 5, pack_name: str = None, is_animated: bool = None) -> dict:
    """Non-interactive search for Telegram emojis. Returns raw metadata mapping. Use Unicode symbols only."""
...
def main():
    logger.info(f"Remoji TG MCP v{VERSION}. Storage: {BASE_DIR}")
